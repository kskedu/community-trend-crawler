...
from news.candidates import build_display_articles, displayed_articles
...
def build_ranked_entry(
    ...
    노출 집합 파생(dedup → filter → [:max])은 candidates.displayed_articles() 단일
    진실원에 있다. URL dedup 은 입력 순서를 보존하므로 여기서 재정렬 없이도 relevance/
    primary cluster 우선 순서가 유지되고, filter 가 incidental/저관련 기사를 기본
    제외한다(부족하면 ARTICLES_MIN 하한 보호를 위해 relevance 높은 순 보충).
    ...
    articles = displayed_articles(raw_articles, max_articles)
    summary, summary_type = summarize(keyword, articles)

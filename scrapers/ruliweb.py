...
class RuliwebScraper(BaseScraper):
    ...
    def scrape(self) -> List[Post]:
        posts = []
        try:
            html = self.fetch(f"{BASE_URL}/best/board/300143")
            soup = BeautifulSoup(html, "html.parser")

            for item in soup.select("tr.table_body.best")[:MAX_POSTS_PER_SITE]:
                ...
                upvotes = self._int(item.select_one(".recomd"))
                comments = self._int(item.select_one(".num_reply .num"))
            ...
            logger.error(f"[ruliweb] 스크래핑 실패: {e}")
